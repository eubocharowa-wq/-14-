...
import { formatsPageData, formatCards } from '@/content/formats';
import { seoData } from '@/content/seo';

export default function FormatsPage() {
  const seo = seoData['/formats'];
  const d = formatsPageData;
  ...
  return (
    <>
      ...
      <Breadcrumbs
        items={[{ label: 'Главная', to: '/' }, { label: 'Форматы сопровождения' }]}
      />
      ...
      <section className="page-section-lift">
        <Container>
          <SectionHeading title={d.chooser.title} />
          <ul className="max-w-2xl space-y-3">
            {d.chooser.items.map((item) => (
